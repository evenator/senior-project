...
{
  static int32_t x = 0, y = 0, omega=0, vert=0;
  static bool_t refresh_values = FALSE;
  ssize_t res;
  static struct js_event js_e_buffer[64];
    ...
    else if(js_e_buffer[idx].type & JS_EVENT_AXIS )// Event Axis detected
    {
      ...
      switch( js_e_buffer[idx].number )
      {
        ...
        case PAD_Y:
          ...
          break;
        case PAD_TWIST:
          omega = (js_e_buffer[idx].value + 1) >>15;
          break;
        case PAD_HAT_Y:
          vert = (js_e_buffer[idx].value +1) >>15;
          break;
        default:
          break;
      }
  ...
  if(refresh_values)// Axis values to refresh
    {
		 ...
		 if( input_state->r2 )
		 {
			 ...
			 // We are trimming so we don't want to update ardrone position
			 x = 0;
			 y = 0;
		 }
		 //Send Command Here:
		 ardrone_at_set_progress_cmd( 1, x, y, -vert/2, omega );
...
//  Crippled since we don't use it (never even searches for a device)
///////////////////////////////

C_RESULT open_ps3pad(void)
{
    C_RESULT res = C_FAIL;
    PRINT("Not checking for PS3 devices.\n");
  return res;
}
