...
DEFINE_THREAD_ROUTINE( stream_loop, data )
{
	...
	//struct video_controller_t * buffer;
	
	//struct sockaddr_in fromm;
    //socklen_t fromm_len = sizeof(fromm);

	while(gAppAlive)
	{
		int size;
		size = recvfrom(sockfd, video_controller_get_stream_ptr(&controller), DATA_RECEIVED_MAX_SIZE, 0, (struct sockaddr *)from,(socklen_t*)&addr_in_size);
		//*size = recvfrom(sockfd, (char*)buffer, *size, MSG_NOSIGNAL, (struct sockaddr*)&fromm, &fromm_len );
		if (size == -1)
		{
			INFO("Socket Return = -1 and Error = %s\n", strerror(errno));
		}
