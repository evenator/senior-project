...
class UI(QtGui.QMainWindow):
	...
	def controller_read(self):
		...
		try:
			if (self.__mode == 2):
				...
				#Dead Band Stick Implementation	
	...
	def display_nav_data(self):
		...
		self.yawScene.clear()
		self.rollScene.clear()
		self.pitchScene.clear()
		self.graphScene.clear()
		
		
		alt = round(navData.get("z","--"),2)
		yaw = round(navData.get("yaw","--"),2)
		pitch = round(navData.get("pitch","--"),2)
		roll = round(navData.get("roll","--"),2)
		
		
		self.ui.findChild(QtGui.QLabel, "Altitude").setText(str(alt))
		...
		self.ui.findChild(QtGui.QLabel, "Yaw").setText(str(yaw))
		self.ui.findChild(QtGui.QLabel, "Pitch").setText(str(pitch))
		self.ui.findChild(QtGui.QLabel, "Roll").setText(str(roll))
		...
		rollPen = QtGui.QPen()
		pitchPen = QtGui.QPen()
		circlePen = QtGui.QPen()
		altPen.setColor(QtGui.QColor("yellow"))
		altPen.setWidth(2)
		yawPen.setColor(QtGui.QColor("red"))
		yawPen.setWidth(2)
		pitchPen.setColor(QtGui.QColor("green"))
		pitchPen.setWidth(2)
		rollPen.setColor(QtGui.QColor("blue"))
		rollPen.setWidth(2)
		circlePen.setColor(QtGui.QColor("black"))
		circlePen.setWidth(1)
		self.yawCircle = self.yawScene.addEllipse(0,0,80,80, circlePen)
		self.pitchCircle = self.pitchScene.addEllipse(0,0,80,80, circlePen)
		self.rollCircle = self.rollScene.addEllipse(0,0,80,80, circlePen)
		#print self.yawCircle.boundingRect()
		#round(navData.get("yaw","--"),2)
		
		#draw graph
		
		self.pointList.append([alt,yaw,pitch,roll])
		if (len(self.pointList) > 26):
			del self.pointList[0]
			
		for i in range(len(self.pointList)-1):
			self.graphScene.addLine( 20*i, 105-int(self.pointList[i][0]/20), 20*(i+1), 105-int(self.pointList[i+1][0]/20),altPen)
			self.graphScene.addLine( 20*i, 105-int(25*self.pointList[i][1]), 20*(i+1), 105-int(25*self.pointList[i+1][1]),yawPen)
			self.graphScene.addLine( 20*i, 105-int(25*self.pointList[i][2]), 20*(i+1), 105-int(25*self.pointList[i+1][2]),pitchPen)
			self.graphScene.addLine( 20*i, 105-int(25*self.pointList[i][3]), 20*(i+1), 105-int(25*self.pointList[i+1][3]),rollPen)
		...
		#draw compass points
		self.yawLine = self.yawScene.addLine(40,40, 40+40*math.sin(round(navData.get("yaw","--"),2)), 40-40*math.cos(round(navData.get("yaw","--"),2)), yawPen)
		self.pitchLine = self.pitchScene.addLine(40,40, 40+40*math.sin(round(navData.get("pitch","--"),2)), 40-40*math.cos(round(navData.get("pitch","--"),2)), pitchPen )
		self.rollLine = self.rollScene.addLine(40,40, 40+40*math.sin(round(navData.get("roll","--"),2)), 40-40*math.cos(round(navData.get("roll","--"),2)), rollPen )
	...
	def display_graph(self):
		self.graphScene.addLine(1,105,500,105)
	...
	def export_map(self):
		print "Exporting map from Mapper to file..."
	...
	def __init__(self):
		self.uiResolution = "mainwindowLow.ui"
		self.ui = uic.loadUi(self.uiResolution)
...
#			self.update_tilt_slider)
		...
		self.maunalImage = QtGui.QPixmap()
		self.maunalImage.load('DroneControl.PNG')
		self.manualScene = QtGui.QGraphicsScene()
		self.manualScene.addPixmap(self.maunalImage)
		self.ui.findChild(QtGui.QGraphicsView, "manualControlView").setScene(self.manualScene)

		self.yawScene = QtGui.QGraphicsScene(0, 0, 80, 80)
		self.pitchScene = QtGui.QGraphicsScene(0, 0, 80, 80)
		self.rollScene = QtGui.QGraphicsScene(0, 0, 80, 80)
		self.graphScene = QtGui.QGraphicsScene(0 , 0, 496, 200)
...
#		self.ui.findChild(QtGui.QGraphicsView, "graphicsView_2").drawBackground()
		...
		vidfile = "videos/"+time.strftime('%Y-%m-%d_%T')+".avi"
		#print vidfile
#		self.video_writer = cv.CreateVideoWriter(vidfile, cv.CV_FOURCC('M','J','P', 'G'), 10, (320,240))
...
def getVideoData(self, a):
	...
	#arr = numpy.fromstring(a, numpy.uint8)
   	...
   	pixmap = Qt.QPixmap.fromImage(img)
   	scene = Qt.QGraphicsScene()
   	if (self.uiResolution == "mainwindowLow.ui"):
   		pixmap = pixmap.scaledToHeight(200)
   	scene.addPixmap(pixmap)
   	self.ui.DownVideo.setScene(scene)
   	
   	#Write to AVI code -- does not work
   	"""
   	iplImg = cv.CreateImage((320,240), cv.IPL_DEPTH_8U, 3)
   	cv.SetData(iplImg, a)
   	#print iplImg 
   	cv.ShowImage("Test", iplImg)
   	
   	#cv.SaveImage("videos/"+time.strftime('%Y-%m-%d_%T')+".png", iplImg)
   	
   	
   	r = cv.WriteFrame(self.video_writer, iplImg)
   	cv.WaitKey(5)
   	print r
   	"""
