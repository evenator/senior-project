...
class UI(QtGui.QMainWindow):
    ...
    def controller_read(self):
    	...
    	try:
            if (self.__mode == 2):
                ...
                if (self.__RY != 0.0):
                	self.__RY = -self.__RY      
                self.pso_socket.update(self.__LY, self.__LX, self.__RX, self.__RY)
                print "update", self.__LY, self.__LX, self.__RX, self.__RY
    ...
    def get_navdata(self, event):
    	nav = self.pso_socket.get_state()
    	print "HEY HEY HEY ITS RIGHT HERE: ", nav	
    ...
    def __init__(self):
		...
		self.UITimer.timerEvent = self.get_navdata
        ...
        #self.connect(self.ui.pushButton_5, SIGNAL("clicked()"), self.setManual)
